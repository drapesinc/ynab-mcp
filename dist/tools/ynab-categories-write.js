        ...
        switch (action) {
            case "create": {
                ...
                if (!group.includes('-') || group.length < 30) {
                    ...
                    const matchedGroup = categoriesResponse.data.category_groups.find(g => g.name.toLowerCase() === group.toLowerCase());
                    if (!matchedGroup) {
            ...
            case "move": {
                ...
                if (fromCurrent < milliunits) {
                    return createErrorResponse(`Insufficient funds in '${from_category}'. Available: ${formatAmount(fromCurrent, currencyCode)}, Requested: ${formatAmount(milliunits, currencyCode)}`);
                }
