...
export interface Profile {
  ...
  defaultAccounts: Record<string, string>; // budget alias -> account name
}
 ...
 */
function parseBudgetAliases(envValue: string | undefined): BudgetAlias[] {
  if (!envValue) return [];
 ...
 */
function parseDefaultAccounts(): Record<string, string> {
...
export function loadConfig(): Config {
  ...
  if (profileNames.length === 0) {
    ...
    let budgets: BudgetAlias[] = parseBudgetAliases(process.env.YNAB_BUDGET_ALIASES);
